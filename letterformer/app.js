...
function renderLabels(documentData, isInteractive = true) {
  const preset = labelPresets[state.labelPreset];
  ...
  const cells = Array.from({ length: preset.total }, (_, index) => {
    ...
    const type = isInteractive ? ' type="button"' : "";
    const data = isInteractive ? ` data-label-index="${index}"` : "";
    return `<${tagName}${type} class="label-cell ${isSelected ? "selected" : ""}"${data}>${isSelected ? address : ""}</${tagName}>`;
  }).join("");
...
function refreshDocument() {
  ...
  fields.printLabelStage.innerHTML = `<article class="label-preview ${labelPresets[state.labelPreset].className}">${renderLabels(state.document, false)}</article>`;
...
function printDocumentFor(mode) {
  if (!state.document) return null;

  if (mode === "print-envelope") {
    return {
      title: "LetterFormer Umschlag",
      pageRule: "size: 220mm 110mm; margin: 0;",
      bodyClass: "print-envelope",
      content: `<article class="envelope-preview">${renderEnvelope(state.document)}</article>`,
    };
  }

  if (mode === "print-labels") {
    return {
      title: "LetterFormer Etiketten",
      pageRule: "size: A4; margin: 0;",
      bodyClass: "print-labels",
      content: `<article class="label-preview ${labelPresets[state.labelPreset].className}">${renderLabels(state.document, false)}</article>`,
    };
  }

  return {
    title: "LetterFormer Brief",
    pageRule: "size: A4; margin: 0;",
    bodyClass: "print-letter",
    content: `<article class="letter-page">${renderLetter(state.document, false)}</article>`,
  };
}

function isolatedPrintCss(pageRule) {
  return `
    @page { ${pageRule} }
    * { box-sizing: border-box; }
    ...
    .subject-line {
      margin: 0 0 ${letterSpacing.subjectAfter};
      font-weight: 700;
    }
    .salutation { margin: 0 0 ${letterSpacing.paragraphAfter}; }
    .letter-body p { margin: 0 0 ${letterSpacing.paragraphAfter}; }
    .closing { margin-top: ${letterSpacing.closingBefore}; }
    .signature-image {
      display: block;
      max-width: 24%;
      max-height: 22mm;
...
function printMode(mode) {
  const printDocument = printDocumentFor(mode);
  ...
  frameDocument.write(`
    ...
    <html lang="${state.language}">
      <head>
        ...
        <style>${isolatedPrintCss(printDocument.pageRule)}</style>
      </head>
      <body class="${printDocument.bodyClass}">
...
function isolatedPrintCss(pageRule) {
  return `
    ...
    * { box-sizing: border-box; }
    ...
    .salutation { margin: 0 0 1em; }
    .letter-body p { margin: 0 0 1em; }
    .closing { margin-top: 0.4em; }
    .signature-image {
      display: block;
      max-width: 24%;
      max-height: 22mm;
